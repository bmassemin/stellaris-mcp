		...
		for _, did := range p.Districts {
			...
			for _, zid := range d.Zones {
				if z, ok := gs.Zones[zid]; ok && z.Type != "zone_default" {
					specs = append(specs, z.Type)
				}
			}
			if len(specs) > 0 {
				fmt.Fprintf(&b, "  %s%s — slots: %s\n", d.Type, lvl, strings.Join(specs, ", "))
			} else {
				fmt.Fprintf(&b, "  %s%s\n", d.Type, lvl)
			}
	...
	if len(p.Deposits) > 0 {
		...
		for _, did := range p.Deposits {
			...
			if isBlocker(dep.Type) {
				...
				features = append(features, dep.Type)
			}
		}
		if len(features) > 0 {
...
func districtSummary(gs *gamestate.GameState, p gamestate.Planet) string {
	...
	labels := map[string]string{
		...
		"district_mining":    "mining",
		"district_farming":   "farming",
		"district_nexus":     "nexus",
		"district_hive":      "hive",
		"district_industrial": "indust",
		"district_research_unity": "research",
	}
	for dtype, n := range counts {
		label := dtype
		if short, ok := labels[dtype]; ok {
			label = short
		}
